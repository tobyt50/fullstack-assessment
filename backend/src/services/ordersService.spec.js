...
describe("Core Backend Production Constraints", () => {
  ...
  it("prevents stock overselling under concurrent requests (Race Condition Fix)", async () => {
    // 1. Setup a dummy product with exactly 1 item in stock
    const { rows } = await pool.query(
      `INSERT INTO products (sku, name, price, stock) VALUES ('TEST-SKU-CONC', 'Test Concurrency', 10.00, 1) RETURNING id`
    );
    const productId = rows[0].id;

    // 2. Define the exact same order request
    const makeOrder = () =>
    ...
    // 3. Fire 5 requests completely concurrently (simulating rapid network traffic)
    ...
    // 4. Assert exact production safety: only ONE checkout should win the race
    expect(successes.length).toBe(1);
    expect(conflicts.length).toBe(4);

    // 5. Verify the stock is exactly 0 and never drops into negative numbers
  ...
  it("prevents double-charging via idempotency constraints", async () => {
    ...
    // 2. Define a charge request using the same idempotency key
    const chargeOrder = () =>
      request(app)
    ...
    // 3. Fire concurrent charges simultaneously
    await Promise.all([chargeOrder(), chargeOrder(), chargeOrder()]);

    // 4. Check the database to ensure exactly ONE payment record was written
